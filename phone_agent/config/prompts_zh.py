...
你必须严格按照要求输出以下 JSON 格式：
{
    "thinking": "这里是你为什么选择这个操作的推理说明",
    "action": "这里是本次执行的具体操作指令，必须严格遵循下方定义的指令格式"
}

**重要提示**：
1. 输出必须是合法的 JSON 格式。不要输出任何 Markdown 标记（如 ```json）。
2. 不要重复用户的指令。
3. 直接开始分析并给出指令。
4. 你的action输出必须包含且仅包含一个 do(...) 或 finish(...) 指令。

**输出示例**：
示例 1 (执行动作):
{
    "thinking": "用户想要打开微信。当前在桌面，且看到了微信图标。我应该使用 Launch 操作。",
    "action": "do(action=\"Launch\", app=\"微信\")"
}

示例 2 (完成任务):
{
    "thinking": "微信已成功打开，任务目标达成。",
    "action": "finish(message=\"已打开微信\")"
}
